class Avo::Resources::User < Avo::BaseResource
  ...
  def fields
    ...
    field :created_at, as: :date, format: "yyyy-LL-dd", sortable: true
    ...
    field "Number of Messages Sent", as: :number do
      record.messages.count
    rescue
      0
    end

    # Fields for the show page
    if view.show?
      field :phone, as: :text, sortable: true
      field :confirmed_at, as: :date, format: "yyyy-LL-dd"
      field :confirmation_sent_at, as: :date, format: "yyyy-LL-dd"
      field :updated_at, as: :date, format: "yyyy-LL-dd"
      field :remember_created_at, as: :date, format: "yyyy-LL-dd"
      field :reset_password_sent_at, as: :date, format: "yyyy-LL-dd"
      field :invitation_created_at, as: :date, format: "yyyy-LL-dd HH:mm:ss"
      field :invitation_accepted_at, as: :date, format: "yyyy-LL-dd"
      field :invitation_sent_at, as: :date, format: "yyyy-LL-dd HH:mm:ss"
      field "has an invitation token", as: :boolean do
        record.invitation_token?
      end
      field :invited_by_id, as: :id

      # Show messages and conversations related to the user
